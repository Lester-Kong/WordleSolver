def wrongLetters(result, guess):
    ...
    for i in range(5):
        if result[i] == "w":
            wrong_letters.update({i : guess[i]})
...
def partialLetters(result, guess):
    ...
    for i in range(5):
        if result[i] == "y":
            partial_letters.update({i : guess[i]})
...
def correctLetters(result, guess):
    ...
    for i in range(5):
        if result[i] == "g":
            correct_letters.update({i : guess[i]})
    ...
    for x in wrong_letters:
        for word in possible_words:
            if wrong_letters[x] in word and word not in wordsToRemove:                       #if the word contains a wrong letter and the word is not tagged for removal
                if wrong_letters[x] in correct_letters.values():                             #if the wrong letter is also a correct letter in another position
                    for i in range(5):
                        if word[i] == wrong_letters[x] and i not in correct_letters:         #if the current position of the possible word does not contain a correct letter
                            wordsToRemove.append(word)
                            break
                elif wrong_letters[x] in partial_letters.values():
                    partial_letters.update({x : wrong_letters[x]})
                else:
                    wordsToRemove.append(word)
...
def secondWordScore(word, letterOccur):
    secondScore = 0
    for i in range(5):
        secondScore += letterOccur[word[i]]
    return secondScore

def bestWord(possible_words, letterFreq, letterOccur):
    bestWordScore = 0
    secondaryWordScore = 0
    theBestWord = ""
    for word in possible_words:
        if bestWordScore == wordScore(word, letterFreq) and secondaryWordScore < secondWordScore(word, letterOccur):
            secondaryWordScore = secondWordScore(word, letterOccur)
            theBestWord = word
        if bestWordScore < wordScore(word, letterFreq):
            bestWordScore = wordScore(word, letterFreq)
            secondaryWordScore = secondWordScore(word, letterOccur)
            theBestWord = word
    return theBestWord
...
def letterOccur(possible_words):
    alphabet = "abcdefghijklmnopqrstuvwxyz"
    letterOccurArr = {}

    for letter in alphabet:
        letterOccurArr.update({letter : 0})
    for word in possible_words:
        for i in range(5):
            letterOccurArr[word[i]] += 1
        
    return letterOccurArr

def wordleSolver(possible_words):
    frequencies = letterFreq(possible_words)
    occurrences = letterOccur(possible_words)
    restart = 0
    guess = bestWord(possible_words, frequencies, occurrences)
    print("Welcome to your own Wordle Solver.")
    print("When prompted, please enter the results of the word entered.")
    print("Type 'g' for correct letters, 'y' for partial letters, and 'w' for wrong letters.")
    print("A word will then be provided to you.")
    print("Your first word is:", guess)
    print("Please enter the result here:")
    result = input()
    for i in range(6):
        if result == "ggggg":
            print("Congratulations! You beat the Wordle puzzle.")
            break
        elif i == 5:
            print("I'm sorry, it seems we have exceeded 6 guesses.")
            print("You lose.")
            break
        else:
            removeWord(possible_words, result, guess)
            if len(possible_words) == 0:
                print("I am sorry, but it appears that you have run out of possible words. Please try again.")
                break
            frequencies = letterFreq(possible_words)
            occurrences = letterOccur(possible_words)
            guess = bestWord(possible_words, frequencies, occurrences)
